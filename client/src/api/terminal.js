...
export async function openTerminalSession(agentId) {
    ...
    if (!res.ok)
        throw new Error(`无法打开终端会话: ${res.statusText}`);
    return res.json();
}
export async function submitTerminalCommand(sessionId, line) {
    const store = useConnectionStore();
    const profile = store.activeProfile;
    if (!profile)
        throw new Error('未连接到后端服务器');
    ...
    if (!res.ok)
        throw new Error(`命令提交失败: ${res.statusText}`);
    return res.json();
}
export async function closeTerminalSession(sessionId) {
    ...
    if (!res.ok)
        throw new Error(`关闭终端会话失败: ${res.statusText}`);
    return res.json();
}
