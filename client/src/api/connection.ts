 ...
 * GET /auth/me
 */
export async function checkSession(serverUrl: string, sessionToken: string): Promise<boolean> {
  const store = useConnectionStore();
  const url = store.normalizeUrl(serverUrl);
 ...
 */
export async function testConnection(serverUrl: string, apiToken: string) {
  const store = useConnectionStore();
  const url = store.normalizeUrl(serverUrl);
