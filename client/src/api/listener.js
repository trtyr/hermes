...
export async function fetchListeners() {
    ...
    if (!res.ok)
        throw new Error('获取监听器失败: ' + res.statusText);
    return res.json();
}
export async function spawnListener(data) {
    const url = `${getBaseUrl()}/listeners`;
    ...
    if (!res.ok)
        throw new Error('创建监听器失败: ' + res.statusText);
    return res.json();
}
export async function startListener(id) {
    ...
    if (!res.ok)
        throw new Error('启动监听器失败: ' + res.statusText);
    return res.json();
}
export async function stopListener(id) {
    ...
    if (!res.ok)
        throw new Error('停止监听器失败: ' + res.statusText);
    return res.json();
}
export async function deleteListener(id) {
    ...
    if (!res.ok)
        throw new Error('删除监听器失败: ' + res.statusText);
    return res.json();
}
