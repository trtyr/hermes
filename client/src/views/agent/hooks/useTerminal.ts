...
export function useTerminal(agentId: string) {
  const sessionId = ref('');
  const cwd = ref('');
  ...
  function printPrompt() {
    const term = getTerm();
    if (!term || !sessionId.value) return;
    term.write(`\r\n\x1b[1;32mhermes\x1b[0m@\x1b[1;34m${agentId}\x1b[0m:\x1b[1;36m${cwd.value}\x1b[0m$ `);
  }

  async function handleEnterCommand(cmd: string) {
    const term = getTerm();
    if (!cmd) {
      printPrompt();
      return;
  ...
  onMounted(async () => {
    ...
    term.onData((e) => {
      if (isExecuting || !sessionId.value) return; 
