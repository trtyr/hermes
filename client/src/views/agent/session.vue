    ...
    <div class="flex-1 flex flex-col bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
      ...
      >
        ...
        <a-tab-pane key="files" tab="文件" force-render class="h-full">
          <SessionFilesTab ref="filesTabRef" :agent-id="agentId" :agent="agent" />
        </a-tab-pane>

        <a-tab-pane key="screenshot" tab="截屏">
          <SessionScreenshotTab :agent-id="agentId" :is-online="!!agent?.is_online" />
        </a-tab-pane>

        <a-tab-pane key="proxy" tab="代理" force-render class="h-full">
          <SessionProxyTab ref="proxyTabRef" :agent-id="agentId" />
        </a-tab-pane>
...
import {
  ...
  ClockCircleOutlined, DisconnectOutlined, DeleteOutlined,
} from '@ant-design/icons-vue';
import {
  fetchAgentDetail, disconnectAgent,
  deleteAgent, updateBeaconConfig,
} from '@/api/agent';
...
const filesTabRef = ref<InstanceType<typeof SessionFilesTab> | null>(null);
const proxyTabRef = ref<InstanceType<typeof SessionProxyTab> | null>(null);
...
// Terminal hook
const { terminalContainer } = useTerminal(agentId);

// Initialize sub-tabs on tab switch
watch(activeTab, (tab) => {
  if (tab === 'files') {
    filesTabRef.value?.initBrowse();
  }
  if (tab === 'proxy') {
    proxyTabRef.value?.loadProxies();
  }
});

// Sync tab title and load agent
onMounted(async () => {
