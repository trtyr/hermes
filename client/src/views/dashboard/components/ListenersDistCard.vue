  ...
  <div class="bg-white dark:bg-[#1C1E22] rounded-lg border border-gray-200 dark:border-[#14161A] shadow-sm p-6 lg:col-span-1">
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center space-x-2">
        <ApiOutlined class="text-lg text-slate-700 dark:text-slate-300" />
        <h3 class="text-lg font-medium text-slate-800 dark:text-slate-100">监听器与协议</h3>
      </div>
    ...
    <div class="flex mb-5">
      <div class="flex-1 text-center border-r border-slate-200 dark:border-slate-700">
        ...
        <div class="text-xl font-semibold text-slate-800 dark:text-slate-100">{{ stats.listeners.enabled }}</div>
      </div>
      <div class="flex-1 text-center border-r border-slate-200 dark:border-slate-700">
        <div class="text-xs text-slate-500 mb-1">已停用</div>
        <div class="text-xl font-semibold text-slate-800 dark:text-slate-100">{{ stats.listeners.disabled }}</div>
      </div>
    ...
    <div class="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">协议分布</div>
    <div class="space-y-3">
      <div class="flex items-center justify-between">
        ...
        <span class="text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-xs">{{ stats.listeners.by_kind.tcp_json }}</span>
      </div>
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-2 text-sm text-slate-600 dark:text-slate-400">
        ...
        <span class="text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-xs">{{ stats.listeners.by_kind.https_json }}</span>
      </div>
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-2 text-sm text-slate-600 dark:text-slate-400">
        ...
        <span class="text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-xs">{{ stats.listeners.by_kind.private_proto }}</span>
      </div>
    </div>
