<template>
  <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-6 lg:col-span-1">
    <div class="flex items-center space-x-2 mb-6">
      <DesktopOutlined class="text-lg text-slate-700" />
      <h3 class="text-lg font-medium text-slate-800">服务器状态</h3>
    </div>
    
    <div class="space-y-5">
      <div>
        <div class="text-sm text-slate-500 mb-1">主机信息</div>
        <div class="text-base text-slate-800 font-medium break-all">
      ...
      <div>
        <div class="flex justify-between items-end mb-2">
          <div class="text-sm text-slate-500">内存使用 ({{ formatBytes(stats.server.memory.used_bytes) }} / {{ formatBytes(stats.server.memory.total_bytes) }})</div>
      ...
      <div>
        <div class="text-sm text-slate-500 mb-1">负载均衡 (Load Average)</div>
        <div class="flex justify-between">
          <div class="text-center">
            <div class="text-xs text-slate-400 mb-0.5">1 分钟</div>
            <div class="text-base font-medium text-slate-700">{{ stats.server.load_average.one.toFixed(2) }}</div>
          </div>
          <div class="text-center">
            <div class="text-xs text-slate-400 mb-0.5">5 分钟</div>
            <div class="text-base font-medium text-slate-700">{{ stats.server.load_average.five.toFixed(2) }}</div>
          </div>
          <div class="text-center">
            <div class="text-xs text-slate-400 mb-0.5">15 分钟</div>
            <div class="text-base font-medium text-slate-700">{{ stats.server.load_average.fifteen.toFixed(2) }}</div>
          </div>
        </div>
      </div>
    </div>
