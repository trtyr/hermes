...
代理转发在Server端的数据模型包括ProxySessionRecord和ProxyStreamRecord。ProxySessionRecord代表一个SOCKS5代理服务实例，除包含代理ID、绑定的Agent、本地监听地址、状态和活跃流列表外，还持久化存储在SQLite新增的agent_proxy_sessions表中。代理会话的删除操作同时移除内存状态和数据库记录（真删除），Server冷重启后通过bootstrap从数据库恢复未关闭的代理会话记录：
...
一个ProxySession代表一个SOCKS5代理服务实例，可以包含多条并发的数据流（Stream）。每条Stream对应一个到内网目标主机的TCP连接。代理隧道的建立过程与之前描述的类似，区别在于现在代理会话支持完整的生命周期管理：创建时在数据库插入记录，运行期间的状态变更（active_streams计数更新、错误信息记录）通过storage模块持久化，删除时同时清除内存和数据库中的记录。Server冷启动时，bootstrap流程从agent_proxy_sessions表中恢复未关闭的代理会话，将其状态标记为closed以便操作员识别。
...
Hermes使用SQLite作为持久化存储，数据模型围绕六个核心实体展开：Agent（受控端）、Task（任务）、Listener（监听器）、AgentBuild（构建任务）、AuditLog（审计日志）和ProxySession（代理会话）。实体之间的关系如下：
...
graph TD
    ...
    Agent -->|建立| ProxySession[代理会话<br/>agent_proxy_sessions]
    Task -->|父子关系| Task
    Operator[操作者] -->|产生| AuditLog[审计日志<br/>audits]
    Operator -->|创建| Listener
    Operator -->|触发| AgentBuild
    Operator -->|创建| ProxySession
```

从E-R图可以看出：一个Listener可以关联多个AgentBuild记录（基于不同目标平台编译）。一个Agent可以接收多个Task，也可以建立多个ProxySession。Task支持自关联（parent_task_id指向自身表），用于表示广播任务与子任务的关系。ProxySession与Agent之间为N:1关系，每个Agent可以同时打开多个SOCKS5代理隧道。AuditLog独立于其他实体，记录所有操作行为。
